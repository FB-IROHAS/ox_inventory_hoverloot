...
function client.openInventory(inv, data)
    ...
    -- HoverLoot 設定を送信
    SendNUIMessage({
        action = 'setupHoverLoot',
        delay = GetConvarInt('inventory:hoverdelay', 200)
    })
...
local function registerCommands()
	...
	lib.addKeybind({
		name = 'hoverloot',
		description = 'Hover Loot',
		defaultKey = GetConvar('inventory:hoverkey', 'H'),
		onPressed = function()
			if not invOpen then return end
			SendNUIMessage({ action = 'setHoverState', pressed = true })
		end,
		onReleased = function()
			SendNUIMessage({ action = 'setHoverState', pressed = false })
		end
	})
